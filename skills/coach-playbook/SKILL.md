...
| 段 | 做什么 | 用哪些技能 | 产出 |
| --- | --- | --- | --- |
| 1️⃣ 澄清 | 搞清给谁用、解决什么痛点、怎样算成功 | `grill-me` | 一句话价值主张 + 目标用户 + 成功标准 |
| 2️⃣ 产品体验 & 用户路径 | 用户怎么用、有哪些界面、关键转化 | `product-designer`（+ 复杂时 `product-manager`） | 用户流程图 + 屏幕清单 + 关键路径 |
...
| 5️⃣ 实现 | 真的把代码写出来 | `implement`（内部 `tdd`）+ 复杂时 `frontend-architect` | 可运行的产品 |
| 6️⃣ 验收 | 逐项检查是否达标 | `code-review` + `web-design-guidelines` + `writing-guidelines` | 验收报告（通过/需修） |
